---
name: article
description: TheBrief の新規記事を作成するエージェント。WebSearch/WebFetch で最新情報を調査し、図表付きの詳細な記事を生成する。/article スキルから呼び出される。
model: opus
tools:
  - WebSearch
  - WebFetch
...
```
[テーマ] [カテゴリ(任意): ict | ai | economy | trading | retail | automotive]
```

カテゴリが未指定の場合、テーマから最適なカテゴリを判断してください。
