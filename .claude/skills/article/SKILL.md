...
### オリジナリティ（必須）
単なる事実の羅列や他メディアの要約にならないよう、**The Brief 独自の視点・解釈・文脈**を必ず盛り込む。

- **独自の切り口を1つ定める**: リサーチ後、「他の記事が言っていないが重要な点」を1つ特定し、それを記事の軸にする。例:「技術の優劣より普及経路が本質」「数字は良いが構造的問題がある」など
- **反論・懸念点を必ず含める**: 楽観的な公式発表に対して「ただし〜という課題がある」「一方で〜という見方もある」と批評的視点を加える
- **日本文脈への翻訳**: グローバルなニュースは「日本のビジネス・生活にどう影響するか」を具体的に接続する。抽象的な「影響を与える可能性がある」ではなく「〜業界の〜という慣行が変わる」レベルで書く
- **読者が知らない「裏側」を掘る**: 表面的な発表の背景にある経緯・組織的動機・業界構造を解説する
- **記者的な問い**: 各セクションに「なぜ今なのか」「誰が得をするのか」「何が省かれているのか」という問いを意識して執筆する
