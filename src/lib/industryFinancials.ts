/** Industry-level PL/BS/CF profile data (percentage-based) for financial statement pages */

export interface PLProfile {
  cost: number;
  sga: number;
  profit: number;
}
...
export interface IndustryFinancial {
  slug: string;
  label: string;
  labelJa: string;
  marketSize: number;
...
export const industryFinancials: IndustryFinancial[] = [
  {
    slug: "retail",
    label: "Retail",
    labelJa: "小売",
...
/** Get financial data for a specific industry by slug */
export function getIndustryFinancial(slug: string): IndustryFinancial | undefined {
  return industryFinancials.find((i) => i.slug === slug);
}
