export type TrendDirection = "up" | "down" | "flat";
export type IndicatorStatus = "good" | "bad" | "neutral";

export interface EconomicIndicator {
  name: string;
  nameEn: string;
  value: string;
  unit: string;
  trend: TrendDirection;
  status: IndicatorStatus;
  description: string;
  category: "growth" | "prices" | "labor" | "financial" | "trade" | "monetary";
  lastUpdated: string;
  change?: string;
  sparkline?: number[];
}

export const economicIndicators: EconomicIndicator[] = [
  {
    name: "GDP成長率",
    nameEn: "GDP Growth Rate",
    value: "0.7",
    unit: "%",
    trend: "down",
    status: "neutral",
    description: "実質GDP成長率（前年比）。外需の弱さにより成長が鈍化。",
    category: "growth",
    lastUpdated: "2026-03-31",
    change: "前期 1.2%",
    sparkline: [1.8, 1.5, 1.2, 1.0, 0.9, 0.7],
  },
  {
    name: "消費者物価指数 (CPI)",
    nameEn: "CPI (YoY)",
    value: "1.6",
    unit: "%",
    trend: "down",
    status: "good",
    description: "生鮮食品を除くコアCPI（2月）。電気・ガス補助金の再開により約4年ぶりの2%割れ。3月は原油高で再び2%台へ。",
    category: "prices",
    lastUpdated: "2026-03-21",
    change: "前月 2.2%",
    sparkline: [3.1, 2.8, 2.5, 2.3, 2.2, 1.6],
  },
  {
    name: "完全失業率",
    nameEn: "Unemployment Rate",
    value: "2.6",
    unit: "%",
    trend: "down",
    status: "good",
    description: "季節調整値（2月）。前月の2.7%から改善。歴史的な低水準を維持。",
    category: "labor",
    lastUpdated: "2026-03-28",
    change: "前月 2.7%",
    sparkline: [2.5, 2.5, 2.4, 2.5, 2.7, 2.6],
  },
  {
    name: "日経平均株価",
    nameEn: "Nikkei 225",
    value: "50,856",
    unit: "円",
    trend: "down",
    status: "bad",
    description: "トランプ相互関税（日本24%）の発表を受け急落。4月4日に前日比2,644円安（-4.9%）の50,856円。リスクオフの売りが加速。",
    category: "financial",
    lastUpdated: "2026-04-05",
    change: "前日 53,500円",
    sparkline: [42000, 47000, 50000, 57000, 53500, 50856],
  },
  {
    name: "円ドル為替レート",
    nameEn: "USD/JPY",
    value: "155.2",
    unit: "円/ドル",
    trend: "down",
    status: "neutral",
    description: "関税ショックによるリスクオフで円高方向に振れる。159円台→155円台へ急速に円高進行。日銀介入観測も。",
    category: "financial",
    lastUpdated: "2026-04-05",
    change: "前日 157.8円",
    sparkline: [150, 153, 155, 158, 159.4, 155.2],
  },
  {
    name: "経常収支",
    nameEn: "Current Account",
    value: "+9,426",
    unit: "億円",
    trend: "up",
    status: "good",
    description: "2026年1月の経常収支黒字。前年同月比で大幅改善。",
    category: "trade",
    lastUpdated: "2026-03-10",
    change: "前年同月 +6,831億円",
    sparkline: [5200, 5800, 6200, 6831, 8100, 9426],
  },
  {
    name: "貿易収支",
    nameEn: "Trade Balance",
    value: "+573",
    unit: "億円",
    trend: "down",
    status: "neutral",
    description: "2026年2月の貿易収支。黒字は維持するも前年同月比で大幅減。",
    category: "trade",
    lastUpdated: "2026-03-19",
    change: "前月 +1,234億円",
    sparkline: [800, 1500, 1200, 1234, 900, 573],
  },
  {
    name: "日銀政策金利",
    nameEn: "BOJ Policy Rate",
    value: "0.75",
    unit: "%",
    trend: "up",
    status: "neutral",
    description: "1995年9月以来の高水準。4月の追加利上げの可能性も示唆。",
    category: "monetary",
    lastUpdated: "2026-03-19",
    change: "前回 0.50%",
    sparkline: [0, 0, 0.1, 0.25, 0.5, 0.75],
  },
  {
    name: "10年国債利回り",
    nameEn: "10Y JGB Yield",
    value: "2.18",
    unit: "%",
    trend: "down",
    status: "neutral",
    description: "関税ショックでリスクオフが進み、安全資産の国債に資金が流入。長期金利は急低下。",
    category: "monetary",
    lastUpdated: "2026-04-05",
    change: "前日 2.32%",
    sparkline: [1.0, 1.3, 1.7, 2.1, 2.39, 2.18],
  },
  {
    name: "マネーストック M2",
    nameEn: "Money Supply M2",
    value: "1,274.9",
    unit: "兆円",
    trend: "down",
    status: "neutral",
    description: "2026年2月のM2残高。前月比でやや減少。",
    category: "monetary",
    lastUpdated: "2026-03-12",
    change: "前月 1,278.3兆円",
    sparkline: [1260, 1265, 1270, 1275, 1278, 1274.9],
  },
  {
    name: "鉱工業生産指数",
    nameEn: "Industrial Production",
    value: "横ばい",
    unit: "",
    trend: "flat",
    status: "neutral",
    description: "米国関税引き上げの前倒し需要と反動減が交錯。全体としてはほぼ横ばい。",
    category: "growth",
    lastUpdated: "2026-03-28",
    change: "前月比 ±0.0%",
    sparkline: [98, 99, 100, 99, 100, 100],
  },
  {
    name: "消費支出",
    nameEn: "Consumer Spending",
    value: "+0.8",
    unit: "% (前年比)",
    trend: "up",
    status: "neutral",
    description: "実質消費支出は緩やかに回復。賃金上昇が消費を下支え。",
    category: "growth",
    lastUpdated: "2026-03-07",
    change: "前月 +0.3%",
    sparkline: [-1.2, -0.5, 0.1, 0.2, 0.3, 0.8],
  },
  {
    name: "有効求人倍率",
    nameEn: "Job-to-Applicant Ratio",
    value: "1.18",
    unit: "倍",
    trend: "down",
    status: "neutral",
    description: "2026年1月の有効求人倍率。前月の1.20から低下も依然1倍超。",
    category: "labor",
    lastUpdated: "2026-02-28",
    change: "前月 1.20倍",
    sparkline: [1.25, 1.24, 1.22, 1.21, 1.20, 1.18],
  },
];

export interface EconomyNewsItem {
  date: string;
  title: string;
  summary: string;
  category: "policy" | "market" | "trade" | "fiscal" | "labor" | "corporate";
  impact: "positive" | "negative" | "neutral";
  relatedIndicators?: string[];
}

export const economyNews: EconomyNewsItem[] = [
  {
    date: "2026-04-05",
    title: "日経平均が暴落、前日比2,644円安の50,856円　関税ショック直撃",
    summary: "トランプ大統領の相互関税（日本24%）発動を受け、東京市場はパニック売りに。下落率4.9%は2024年8月5日以来の大きさ。自動車・半導体関連株が軒並みストップ安。週明けの更なる下落も懸念。",
    category: "market",
    impact: "negative",
    relatedIndicators: ["Nikkei 225", "USD/JPY"],
  },
  {
    date: "2026-04-05",
    title: "円急伸、一時155円台　関税リスクオフで4円超の円高",
    summary: "トランプ関税ショックによるリスクオフで円が急騰。159円台から一時155円台まで4円超の円高に。輸出企業の採算悪化懸念と安全通貨としての円買いが交錯。",
    category: "market",
    impact: "neutral",
    relatedIndicators: ["USD/JPY", "Trade Balance"],
  },
  {
    date: "2026-04-05",
    title: "原油価格急落、WTI一時105ドル割れ　世界景気後退懸念",
    summary: "トランプ関税による世界的な景気後退懸念から原油先物が急落。WTIは112ドル台から一時104ドル台まで下落。中東情勢の地政学リスクと景気減速懸念が綱引き。",
    category: "trade",
    impact: "neutral",
    relatedIndicators: ["CPI (YoY)", "Trade Balance"],
  },
  {
    date: "2026-04-05",
    title: "政府、関税対策の緊急経済パッケージ策定へ　来週にも閣僚会議",
    summary: "高市首相がトランプ関税への対応として緊急経済パッケージの策定を指示。中小企業の資金繰り支援、サプライチェーン多角化支援、農産品の輸出先転換支援が柱。",
    category: "policy",
    impact: "neutral",
    relatedIndicators: ["Trade Balance", "Nikkei 225"],
  },
  {
    date: "2026-04-04",
    title: "4月の食品値上げ2,798品目、2026年初の値上げラッシュ",
    summary: "帝国データバンク調べで4月の飲食料品値上げは2,798品目（平均14%）。調味料1,514品目、加工食品609品目、酒類・飲料369品目。電気・ガス補助金も3月末で終了し家計への影響大。",
    category: "market",
    impact: "negative",
    relatedIndicators: ["CPI (YoY)", "Consumer Spending"],
  },
  {
    date: "2026-04-04",
    title: "日経平均、53,500円　関税発表を受け後場に急落",
    summary: "トランプ相互関税の詳細発表を受け、後場に入り売りが加速。一時800円超の下落。自動車株を中心に全面安の展開。",
    category: "market",
    impact: "negative",
    relatedIndicators: ["Nikkei 225"],
  },
  {
    date: "2026-04-03",
    title: "春闘3次集計、賃上げ率5.09%　中小も5%到達",
    summary: "連合の第3回集計（2,311組合）で賃上げ率は5.09%。中小企業（300人未満）も5.00%と前年並みの高水準を維持し、3年連続5%超の賃上げが実現。",
    category: "labor",
    impact: "positive",
    relatedIndicators: ["Consumer Spending", "Unemployment Rate"],
  },
  {
    date: "2026-04-02",
    title: "トランプ相互関税から1年 — 日本への影響を振り返る",
    summary: "2025年4月2日の24%相互関税発表から1年。その後7月の日米枠組み合意で15%に引き下げ、2026年2月の最高裁違憲判決でIEEPA関税は無効化。現在はセクション122に基づく暫定10%関税が適用中。新たなセクション301調査も開始。",
    category: "trade",
    impact: "neutral",
    relatedIndicators: ["Trade Balance", "USD/JPY", "Nikkei 225"],
  },
  {
    date: "2026-04-01",
    title: "暫定予算8.6兆円が成立、本予算122.3兆円は4月11日に自然成立見込み",
    summary: "本予算（一般会計122.3兆円、過去最大）は衆院通過済みだが参院審議中のため、4月1〜11日をカバーする暫定予算8.6兆円が成立。本予算は憲法の30日ルールにより4月11日に自然成立の見込み。",
    category: "fiscal",
    impact: "neutral",
  },
  {
    date: "2026-03-21",
    title: "2月コアCPI、前年比+1.6%　約4年ぶり2%割れ",
    summary: "生鮮食品を除くコアCPIは前年同月比+1.6%と2022年3月以来47ヶ月ぶりの2%割れ。電気・ガス補助金の再開が主因。3月は原油高で再び2%台の見込み。",
    category: "policy",
    impact: "positive",
    relatedIndicators: ["CPI (YoY)", "BOJ Policy Rate"],
  },
  {
    date: "2026-03-23",
    title: "春闘1次集計、賃上げ率5.26%　大手で満額回答相次ぐ",
    summary: "連合の第1回集計（1,100組合）で賃上げ率は5.26%。自動車・電機など大手で満額回答が相次ぎ、33年ぶりの高水準を維持。",
    category: "labor",
    impact: "positive",
  },
  {
    date: "2026-03-19",
    title: "日銀、政策金利0.75%で据え置き（8対1）",
    summary: "日銀は金融政策決定会合で政策金利を0.75%に据え置き。8対1で決定（高田委員が1%への利上げを主張し反対）。中東情勢の不透明感を注視しつつ、利上げ継続の姿勢は維持。",
    category: "policy",
    impact: "neutral",
    relatedIndicators: ["BOJ Policy Rate", "10Y JGB Yield", "USD/JPY"],
  },
  {
    date: "2026-03-14",
    title: "トヨタ、EV新ライン稼働で豊田市に3,000億円投資",
    summary: "トヨタ自動車が次世代EV専用ラインの稼働を開始。2028年までにEV生産能力を年間50万台に引き上げ。部品メーカーへの波及効果も期待。",
    category: "corporate",
    impact: "positive",
  },
  {
    date: "2026-03-10",
    title: "1月経常収支、9,426億円の黒字　前年比+38%",
    summary: "海外投資からの第一次所得収支が拡大し、経常収支は前年同月比38%増の黒字を記録。円安による海外利益の膨張が寄与。",
    category: "trade",
    impact: "positive",
    relatedIndicators: ["Current Account", "USD/JPY"],
  },
  {
    date: "2026-03-07",
    title: "実質消費支出、前年比+0.8%で3ヶ月連続増",
    summary: "総務省の家計調査で1月の実質消費支出は前年同月比+0.8%と3ヶ月連続の増加。賃上げ効果が徐々に消費に波及。",
    category: "labor",
    impact: "positive",
  },
  {
    date: "2026-02-28",
    title: "有効求人倍率、1.18倍に低下も高水準維持",
    summary: "1月の有効求人倍率は1.18倍と前月から0.02ポイント低下。人手不足は継続も、一部業種で求人の慎重姿勢が見られる。",
    category: "labor",
    impact: "neutral",
  },
  {
    date: "2026-02-18",
    title: "第2次高市内閣発足、経済政策の継続を表明",
    summary: "衆院選大勝を受け第2次高市内閣が発足。高市首相は「賃上げと投資の好循環を加速する」と表明し、戦略的インフラ投資の強化を掲げる。",
    category: "policy",
    impact: "neutral",
  },
  {
    date: "2026-02-10",
    title: "日経平均、終値5万7,000円超え　衆院選後の最高値更新",
    summary: "衆院選で与党圧勝を受け、海外投資家の日本株選好が加速。日経平均は終値で5万7,000円を突破し、2025年10月の5万円突破からわずか4ヶ月で7,000円上昇。",
    category: "market",
    impact: "positive",
  },
  {
    date: "2025-12-19",
    title: "日銀、政策金利を0.75%に引き上げ",
    summary: "日銀は0.50%から0.75%への追加利上げを決定。1995年9月以来約30年ぶりの高水準。為替市場では一時円高に振れるも、その後円安基調に回帰。",
    category: "policy",
    impact: "neutral",
    relatedIndicators: ["BOJ Policy Rate", "USD/JPY", "10Y JGB Yield"],
  },
];

export const newsCategoryLabels: Record<string, string> = {
  policy: "政策",
  market: "市場",
  trade: "貿易",
  fiscal: "財政",
  labor: "雇用",
  corporate: "企業",
};

export const categoryLabels: Record<string, { ja: string; en: string }> = {
  growth: { ja: "成長・生産", en: "Growth & Production" },
  prices: { ja: "物価", en: "Prices" },
  labor: { ja: "雇用", en: "Labor" },
  financial: { ja: "金融市場", en: "Financial Markets" },
  trade: { ja: "貿易・国際収支", en: "Trade & Balance of Payments" },
  monetary: { ja: "金融政策", en: "Monetary Policy" },
};
