...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      title: article.title,
      description: article.summary,
      url: `https://thebrief.info/industry/daily/${article.slug}`,
      type: "article",
      locale: "ja_JP",
      siteName: "The Brief",
    },
        ...
        if (k % 2 === 1) {
          return (
            <strong key={k} className="font-semibold text-foreground" style={{ textDecorationLine: "none" }}>
              <span className="bg-[#06b6d4]/[0.07] dark:bg-[#06b6d4]/[0.15] px-1 py-0.5 rounded">{part}</span>
            </strong>
          );
        }
...
export default async function ArticlePage({
  ...
  const article = dailyArticles.find((a) => a.slug === articleSlug);
  if (!article) notFound();

  const articleIndex = dailyArticles.findIndex((a) => a.slug === articleSlug);
  const prevArticle = articleIndex > 0 ? dailyArticles[articleIndex - 1] : null;
  const nextArticle = articleIndex < dailyArticles.length - 1 ? dailyArticles[articleIndex + 1] : null;
    ...
    <div className="max-w-[720px] mx-auto px-5 sm:px-6 py-16">
      ...
      <BreadcrumbJsonLd
        items={[
          ...
          { name: "生活・日用品", href: "/industry/daily" },
          { name: article.title, href: `/industry/daily/${article.slug}` },
        ]}
      />
      ...
      <nav className="flex items-center gap-1.5 text-[11px] text-foreground/40 mb-12 font-medium tracking-wide">
        ...
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="opacity-30"><path d="M9 18l6-6-6-6"/></svg>
        <Link href="/industry/daily" className="hover:text-foreground/70 transition-colors">生活・日用品</Link>
      </nav>
      ...
      <header className="mb-14">
        ...
        <p className="mt-6 text-[15px] text-foreground/65 leading-[1.9] border-l-2 border-[#06b6d4]/20 pl-5">{article.summary}</p>
        ...
        <div className="relative p-6 sm:p-8 rounded-2xl border border-foreground/[0.05]">
          ...
          <ol className="space-y-0">
            {article.sections.map((section, i) => (
              <li key={i}>
                <a href={`#section-${i}`} className="group flex items-center gap-4 py-2.5 text-foreground/65 hover:text-[#06b6d4] transition-colors">
                  <span className="text-[11px] tabular-nums font-semibold text-[#06b6d4]/30 group-hover:text-[#06b6d4]/60 w-5 text-right transition-colors">{String(i + 1).padStart(2, "0")}</span>
            ...
            <section key={i} id={`section-${i}`} className="mb-20 scroll-mt-24">
              <div className="mb-8">
                <div className="flex items-center gap-4 mb-3">
                  <span className="text-[32px] sm:text-[40px] font-bold tabular-nums text-[#06b6d4]/[0.08] leading-none select-none font-serif">{String(i + 1).padStart(2, "0")}</span>
                ...
                <div className="h-px bg-gradient-to-r from-[#06b6d4]/15 via-[#06b6d4]/5 to-transparent" />
              </div>
              <div className="space-y-6">
                {paragraphs.map((paragraph, j) => {
                  ...
                  if (trimmed.startsWith("> ")) {
                    ...
                    return (
                      <div key={j} className="my-8 pl-5 py-4 rounded-r-lg bg-[#06b6d4]/[0.03] dark:bg-[#06b6d4]/[0.06] border-l-3 border-[#06b6d4]/25">
                    ...
                    if (bracketEnd > 0) {
                      ...
                      return (
                        <div key={j} className="group pl-5 py-3 relative">
                          <div className="absolute left-0 top-0 bottom-0 w-[3px] rounded-full bg-[#06b6d4]/20 group-hover:bg-[#06b6d4]/35 transition-colors" />
                          <div className="text-[11px] font-bold text-[#06b6d4]/60 tracking-wide mb-1.5">{label}</div>
                  ...
                  return (
                    <p key={j} className={isLead ? "text-[16px] text-foreground/80 leading-[2] tracking-[0.02em] first-letter:text-[2em] first-letter:font-serif first-letter:font-bold first-letter:text-[#06b6d4]/40 first-letter:float-left first-letter:mr-1 first-letter:mt-1 first-letter:leading-[0.8]" : "text-[15px] text-foreground/75 leading-[2] tracking-[0.02em]"}>
      ...
      <div className="mt-20 pt-12 border-t border-foreground/[0.04]">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {prevArticle ? (
            <Link href={`/industry/daily/${prevArticle.slug}`} className="group p-5 rounded-2xl border border-foreground/[0.05] hover:border-foreground/[0.1] transition-all hover:bg-foreground/[0.01]">
          ...
          {nextArticle ? (
            <Link href={`/industry/daily/${nextArticle.slug}`} className="group p-5 rounded-2xl border border-foreground/[0.05] hover:border-foreground/[0.1] transition-all text-right hover:bg-foreground/[0.01]">
        ...
        <div className="mt-8 text-center">
          <Link href="/industry/daily" className="inline-flex items-center gap-2 text-[12px] text-foreground/35 hover:text-foreground/60 transition-colors font-medium tracking-wide">
