...
export async function generateMetadata({
  ...
  const article = pharmaArticles.find((a) => a.slug === slug);
  ...
  return {
    ...
    openGraph: {
      title: article.title,
      description: article.summary,
      url: `https://thebrief.info/industry/pharma/${article.slug}`,
      type: "article",
      locale: "ja_JP",
      siteName: "The Brief",
    },
        ...
        if (k % 2 === 1) {
          return (
            <strong key={k} className="font-semibold text-foreground" style={{ textDecorationLine: "none" }}>
              <span className="bg-[#10b981]/[0.07] dark:bg-[#10b981]/[0.15] px-1 py-0.5 rounded">{part}</span>
            </strong>
          );
        }
...
export default async function ArticlePage({
  ...
  const article = pharmaArticles.find((a) => a.slug === articleSlug);
  if (!article) notFound();

  const articleIndex = pharmaArticles.findIndex((a) => a.slug === articleSlug);
  const prevArticle = articleIndex > 0 ? pharmaArticles[articleIndex - 1] : null;
  const nextArticle = articleIndex < pharmaArticles.length - 1 ? pharmaArticles[articleIndex + 1] : null;
    ...
    <div className="max-w-[720px] mx-auto px-5 sm:px-6 py-16">
      ...
      <BreadcrumbJsonLd
        items={[
          ...
          { name: "医薬品・医療介護", href: "/industry/pharma" },
          { name: article.title, href: `/industry/pharma/${article.slug}` },
        ]}
      />
      ...
      <nav className="flex items-center gap-1.5 text-[11px] text-foreground/40 mb-12 font-medium tracking-wide">
        ...
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="opacity-30"><path d="M9 18l6-6-6-6"/></svg>
        <Link href="/industry/pharma" className="hover:text-foreground/70 transition-colors">医薬品・医療介護</Link>
      </nav>
      ...
      <header className="mb-14">
        ...
        <p className="mt-6 text-[15px] text-foreground/65 leading-[1.9] border-l-2 border-[#10b981]/20 pl-5">{article.summary}</p>
        ...
        <div className="relative p-6 sm:p-8 rounded-2xl border border-foreground/[0.05]">
          ...
          <ol className="space-y-0">
            {article.sections.map((section, i) => (
              <li key={i}>
                <a href={`#section-${i}`} className="group flex items-center gap-4 py-2.5 text-foreground/65 hover:text-[#10b981] transition-colors">
                  <span className="text-[11px] tabular-nums font-semibold text-[#10b981]/30 group-hover:text-[#10b981]/60 w-5 text-right transition-colors">{String(i + 1).padStart(2, "0")}</span>
            ...
            <section key={i} id={`section-${i}`} className="mb-20 scroll-mt-24">
              <div className="mb-8">
                <div className="flex items-center gap-4 mb-3">
                  <span className="text-[32px] sm:text-[40px] font-bold tabular-nums text-[#10b981]/[0.08] leading-none select-none font-serif">{String(i + 1).padStart(2, "0")}</span>
              ...
              <div className="space-y-6">
                {paragraphs.map((paragraph, j) => {
                  ...
                  if (trimmed.startsWith("> ")) {
                    ...
                    return (
                      <div key={j} className="my-8 pl-5 py-4 rounded-r-lg bg-[#10b981]/[0.03] dark:bg-[#10b981]/[0.06] border-l-3 border-[#10b981]/25">
                    ...
                    if (bracketEnd > 0) {
                      ...
                      return (
                        <div key={j} className="group pl-5 py-3 relative">
                          <div className="absolute left-0 top-0 bottom-0 w-[3px] rounded-full bg-[#10b981]/20 group-hover:bg-[#10b981]/35 transition-colors" />
                          <div className="text-[11px] font-bold text-[#10b981]/60 tracking-wide mb-1.5">{label}</div>
                  ...
                  return (
                    <p key={j} className={isLead ? "text-[16px] text-foreground/80 leading-[2] tracking-[0.02em] first-letter:text-[2em] first-letter:font-serif first-letter:font-bold first-letter:text-[#10b981]/40 first-letter:float-left first-letter:mr-1 first-letter:mt-1 first-letter:leading-[0.8]" : "text-[15px] text-foreground/75 leading-[2] tracking-[0.02em]"}>
      ...
      <div className="mt-20 pt-12 border-t border-foreground/[0.04]">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {prevArticle ? (
            <Link href={`/industry/pharma/${prevArticle.slug}`} className="group p-5 rounded-2xl border border-foreground/[0.05] hover:border-foreground/[0.1] transition-all hover:bg-foreground/[0.01]">
          ...
          {nextArticle ? (
            <Link href={`/industry/pharma/${nextArticle.slug}`} className="group p-5 rounded-2xl border border-foreground/[0.05] hover:border-foreground/[0.1] transition-all text-right hover:bg-foreground/[0.01]">
        ...
        <div className="mt-8 text-center">
          <Link href="/industry/pharma" className="inline-flex items-center gap-2 text-[12px] text-foreground/35 hover:text-foreground/60 transition-colors font-medium tracking-wide">
