...
import { pharmaCompanies } from "@/lib/pharmaCompanies";
...
export default function Page() {
  const sortedArticles = [...pharmaArticles].sort((a, b) => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <BreadcrumbJsonLd items={[{ name: "Home", href: "/" }, { name: "Industry", href: "/industry" }, { name: "医薬品・医療介護", href: "/industry/pharma" }]} />
      ...
      <div className="mb-10">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h1 className="font-serif text-2xl md:text-3xl font-bold">医薬品・医療介護</h1>
            <span className="text-[10px] tracking-[2px] uppercase text-foreground/50">Pharma & Healthcare</span>
          </div>
        </div>
        <p className="mt-3 text-sm text-foreground/60 leading-relaxed">新薬開発、医療DX、バイオテクノロジー、介護・ヘルスケアの最新動向。</p>
      </div>
      <div className="h-px bg-brief-border mb-8" />
      <IndustryFinancialNav slug="pharma" />

      <CompetitorCards companies={pharmaCompanies} title="日本の主要医薬品企業" />

      <div className="h-px bg-brief-border my-8" />

      <div className="space-y-4">
        {sortedArticles.map((article) => (
          <Link key={article.slug} href={`/industry/pharma/${article.slug}`} className="group block p-5 rounded-xl border border-brief-border hover:border-[#10b981]/30 bg-brief-card transition-all">
            ...
            <h2 className="font-serif text-lg font-bold leading-snug group-hover:text-[#10b981] transition-colors">{article.title}</h2>
            <p className="mt-2 text-sm text-foreground/55 leading-relaxed line-clamp-2">{article.summary}</p>
            <div className="mt-3 flex flex-wrap gap-1.5">
              {article.tags.map((tag) => (<span key={tag} className="text-[9px] px-2 py-0.5 rounded-full bg-[#10b981]/8 text-[#10b981]/70 font-medium">{tag}</span>))}
            </div>
          </Link>
        ))}
      </div>
