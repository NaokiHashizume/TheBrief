...
import { materialsCompanies } from "@/lib/materialsCompanies";
...
export default function Page() {
  const sortedArticles = [...materialsArticles].sort((a, b) => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <BreadcrumbJsonLd items={[{ name: "Home", href: "/" }, { name: "Industry", href: "/industry" }, { name: "素材", href: "/industry/materials" }]} />
      ...
      <div className="mb-10">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ backgroundColor: "#0ea5e915", color: "#0ea5e9" }}>
          ...
          <div>
            <h1 className="font-serif text-2xl md:text-3xl font-bold">素材</h1>
      ...
      <CompetitorCards companies={materialsCompanies} title="日本の主要素材企業" />
      ...
      <div className="space-y-4">
        {sortedArticles.map((article) => (
          <Link key={article.slug} href={`/industry/materials/${article.slug}`} className="group block p-5 rounded-xl border border-brief-border hover:border-[#0ea5e9]/30 bg-brief-card transition-all">
            ...
            <h2 className="font-serif text-lg font-bold leading-snug group-hover:text-[#0ea5e9] transition-colors">{article.title}</h2>
            <p className="mt-2 text-sm text-foreground/55 leading-relaxed line-clamp-2">{article.summary}</p>
            <div className="mt-3 flex flex-wrap gap-1.5">
              {article.tags.map((tag) => (<span key={tag} className="text-[9px] px-2 py-0.5 rounded-full bg-[#0ea5e9]/8 text-[#0ea5e9]/70 font-medium">{tag}</span>))}
            </div>
          </Link>
        ))}
      </div>
