...
import {
  AiGovLawOverviewDiagram,
  AiGovTransparencyDiagram,
  AiGovRiskMatrixDiagram,
  AiGovInventoryDiagram,
  AiGovExamplesDiagram,
  AiGovStrategicDiagram,
} from "@/components/AiGovernanceDiagrams";

const SLUG = "ai-governance-post-basiclaw-2026";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: article.title, description: article.summary, url: `https://thebrief.info/industry/ai/${SLUG}`, type: "article", locale: "ja_JP", siteName: "The Brief" },
        ...
        return (
          <span key={k}>
            {quoteParts.map((qp, qi) => {
              if (qp.startsWith("「") && qp.endsWith("」")) return <span key={qi} className="text-foreground/90 font-medium">{qp}</span>;
...
const diagramMap: Record<string, React.FC> = {
  "ai-gov-law-overview": AiGovLawOverviewDiagram,
  "ai-gov-transparency": AiGovTransparencyDiagram,
  "ai-gov-risk-matrix": AiGovRiskMatrixDiagram,
  "ai-gov-inventory": AiGovInventoryDiagram,
  "ai-gov-examples": AiGovExamplesDiagram,
  "ai-gov-strategic": AiGovStrategicDiagram,
};
            ...
            <section key={i} id={`section-${i}`} className="mb-20 scroll-mt-24">
              ...
              <div className="space-y-6">
                {paragraphs.map((paragraph, j) => {
                    ...
                    if (bracketEnd > 0) {
                      return (
                        <div key={j} className="group pl-5 py-3 relative">
                          <div className="absolute left-0 top-0 bottom-0 w-[3px] rounded-full bg-[#8b5cf6]/20" />
                          <div className="text-[11px] font-bold text-[#8b5cf6]/60 tracking-wide mb-1.5">{trimmed.slice(1, bracketEnd)}</div>
                          <p className="text-[14px] text-foreground/75 leading-[1.9]"><RichText text={trimmed.slice(bracketEnd + 1).trim()} /></p>
                        </div>
                      );
                    }
                  }
                  const isLead = j === 0;
                  return (
                    <p key={j} className={isLead ? "text-[16px] text-foreground/80 leading-[2] tracking-[0.02em]" : "text-[15px] text-foreground/75 leading-[2] tracking-[0.02em]"}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {prevArticle ? (
            <Link href={`/industry/ai/${prevArticle.slug}`} className="group p-5 rounded-2xl border border-foreground/[0.05] hover:border-foreground/[0.1] transition-all hover:bg-foreground/[0.01]">
              <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-[2px] text-foreground/30 mb-2 font-medium"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="opacity-40"><path d="M15 18l-6-6 6-6"/></svg>Previous</div>
          ...
          {nextArticle ? (
            <Link href={`/industry/ai/${nextArticle.slug}`} className="group p-5 rounded-2xl border border-foreground/[0.05] hover:border-foreground/[0.1] transition-all text-right hover:bg-foreground/[0.01]">
              <div className="flex items-center justify-end gap-1.5 text-[10px] uppercase tracking-[2px] text-foreground/30 mb-2 font-medium">Next<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="opacity-40"><path d="M9 18l6-6-6-6"/></svg></div>
