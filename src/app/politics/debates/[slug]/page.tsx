  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="flex items-center gap-2 flex-wrap mb-3">
        ...
        <span className="text-xs text-foreground/30">{debate.committee}</span>
      </div>
        ...
        <div className="border border-brief-border dark:border-white/5 rounded-xl overflow-hidden divide-y divide-brief-border dark:divide-white/5">
          {debate.parties.map((p) => (
            <div key={p.name} className="px-5 py-3 flex items-center justify-between gap-4">
              ...
              <span className="text-xs text-foreground/40 text-right">{p.position}</span>
            </div>
          ))}
        </div>
      </div>
            ...
            return (
              <div key={t.date} className="relative flex gap-4 pb-5 last:pb-0">
                ...
                <div>
                  ...
                  <p className={`text-sm mt-0.5 ${isLatest ? "text-foreground font-medium" : "text-foreground/50"}`}>
                    {t.event}
                  </p>
                </div>
              </div>
