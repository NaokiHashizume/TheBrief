...
async function generateTweetWithClaude(article) {
  ...
  const basePrompt = `あなたは X (旧 Twitter) でバズるコンテンツを作るプロのライターです。
以下の手順で、TheBrief の記事を元にした「高インプレッション投稿」を作成してください。
...
【ステップ 3: 高インプレッション投稿の生成】
バズる投稿の鉄則を使って書いてください:

◆ 冒頭フック (最重要): 最初の 1〜2 行で「続きを読みたい」と思わせる
  良い例:
  - 「実は〇〇だった」「多くの人が知らない〇〇」
  - 具体的な数字・事実 (「〇〇が▲▲%増」「〇〇兆円規模」)
  - 問いかけ (「なぜ〇〇が今これほど注目されているのか？」)
  - 驚き・逆説 (「〇〇なのに、実は〇〇」)
  悪い例: 「〇〇についての解説です」「〇〇を紹介します」(→ 読まれない)

◆ 本文: 記事の最も鋭い insight・数字・背景を 1 文で凝縮する

◆ 誘導: 「👇」「続きはこちら」でクリックを促す

◆ 地政学フック (該当する場合): ウクライナ / イラン / 中東 / 米中対立 / 半導体安保 / 台湾有事 /
  エネルギー / 日銀 / 円安 などに絡められる論点があれば必ず盛り込む (インプレッション大幅増)

【厳守ルール】
- X 重み 250 以下 (日本語=2、URL=23固定、ASCII=1)
- 本文中に ${article.url} をそのまま 1 回だけ含める
...
【出力フォーマット (厳守)】
<tweet>ここに本文</tweet> で囲む。タグ内は投稿本文のみ。`;
