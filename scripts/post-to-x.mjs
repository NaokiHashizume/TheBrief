...
function buildTweet(article) {
  // X はハッシュタグも文字数に含む。記事の tags から最大 8 個。
  const tagPool = (article.tags || []).filter((t) => t && t.length <= 12);
  const picked = tagPool.slice(0, 8).map((t) => `#${t.replace(/\s+/g, "")}`);
...
【このアカウントの読者傾向 (重要)】
- 地政学・国際情勢系のキーワードに強く反応する。特に ウクライナ情勢 / イラン / 中東 / 米中対立 / ロシア / 北朝鮮 / 半導体 安保 / 台湾有事 など。
...
- ハッシュタグは 5〜8 個 (多め)。記事と読者の関心に合致する具体的で検索されやすいキーワードを幅広く選ぶ。
  良い例: #ウクライナ情勢 #イラン #中東情勢 #米中対立 #半導体 #台湾有事 #日銀 #円安 #エネルギー安保 #地政学 #国際情勢 #安全保障 #トランプ #NATO など
  避ける: #ニュース #日本 #経済 のような一般的すぎるもの、#TheBrief のようなブランド名 (= 付けない)
  ハッシュタグの分だけ重みも増えるので、本文を短めにして調整すること
- 絵文字は 1〜3 個まで
