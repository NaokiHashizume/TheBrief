...
【ステップ 3: クリック誘導型投稿の生成】
目的は「続きを読みたい」と思わせてリンクをクリックさせること。答えはツイート内で完結させない。

◆ 冒頭フック (最重要・1 行目で勝負が決まる):
  良い例:
  - 「なぜ〇〇が今これほど注目されているのか？」
  - 「〇〇の裏に隠された本当の理由」
  - 「売上の99%が嘘だった——〇〇で何が起きているのか」
  - 「多くの人が知らない〇〇の実態」
  - 「〇〇兆円規模の変化が静かに始まっている」
  禁止: 「〇〇です」「〇〇しました」で終わる平叙文 (= クリックされない)

◆ 本文 (1 文のみ): 核心に「触れるが答えは出さない」。問いを立てて終わる
  良い例: 「背景には▲▲という構造的な問題がある——」「詳細はリンク先で👇」
  禁止: 答えを書き切る、「〜です。〜です。」と説明を完結させる

◆ CTA: 「続きはこちら👇」「全文はこちら→」を本文直後に入れ、その直後に URL

◆ 地政学フック (該当する場合): ウクライナ / イラン / 中東 / 米中対立 / 半導体安保 / 台湾有事 /
  エネルギー / 日銀 / 円安 などと絡められれば必ず盛り込む (インプレッション大幅増)
...
<tweet>ここに本文 ${article.url} #ハッシュタグ</tweet> の形で囲む。URL は必ずタグ内に含める。`;
  ...
  const retryPrompt = `以下の記事へのクリックを促す短いツイートを作成してください。

タイトル: ${article.title}
URL: ${article.url}

ルール:
- 冒頭に強いフック (「なぜ〇〇なのか？」「〇〇の本当の理由」「知らないと損する〇〇」など)
- 答えはツイート内で言い切らない。続きを読みたくさせる
- 末尾に「続きはこちら👇」などの CTA
- その直後に ${article.url} を必ず含める
- ハッシュタグ 2〜3 個
